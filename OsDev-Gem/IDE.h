...
class IDE : public Singleton<IDE>
{
public:
	IDE();
	virtual ~IDE();
	...
	void WaitUntilReady(uint16 iBase);
	
	// Cette fonction permet de lire un buffer sur le disque
	void Read(uint16 iDrive, uint16 iNumblock, uint16 iCount, char* sBuffer);
	
	// Cette fonction permet d'ecrire un buffer sur le disque
	void uint16 Write(uint16 iDrive, uint16 iNumblock, uint16 iCount, const char* sBuffer)
